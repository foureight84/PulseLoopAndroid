...
/**
 * Ported from CoachSummaryPromptBuilder.swift.
 * Prompts for the Today/Sleep coach-card summaries.
 */
object CoachSummaryPromptBuilder {
    fun systemPrompt(kind: CoachSummaryKind): String {
        val focus = when (kind) {
            CoachSummaryKind.TODAY ->
                "This card sits on the Today page. Give a quick, motivating read on how the day is going so far (steps, heart rate, sleep last night, activity)."
            CoachSummaryKind.SLEEP_DAY ->
                "This card sits on the Sleep page for last night. Interpret the night — duration, deep/light/awake balance, and the sleep score — and what it means for today."
            else ->
                "This card sits on the Sleep page for a multi-night range. Summarize the sleep trend (average duration, consistency, score) over the period."
        }
        return """
