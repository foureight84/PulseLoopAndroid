...
> (2026-07-17/18) — **#65a**, **#65b**, and now **#65c DONE** (`34d0494`, `MIGRATION_10_11` adds
> `label`/`statusRaw`/`sequence` to `coach_tool_calls` + a `CoachToolTraceDisclosure` Composable),
> all three runtime-verified on `emulator-5554` including real schema upgrades over existing data
> (v9→v10, then v10→v11). #65b also fixed a real (non-iOS-parity) bug found along the way:
> `OpenAIResponse.parse` threw on any real API response with output content — see the #65b row for
> detail. **Next: #65d (environment/weather context)** — net-new, Open-Meteo + the already-present
> `FusedLocationProviderClient` dep, cached in SharedPreferences/DataStore — then #65f (variety
> hints + notification sleep-gating). #65e (context-budget compact mode) has no Android equivalent
> yet and should stay SKIPped unless a constrained/offline provider is added.
>
