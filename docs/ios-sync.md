...
> (2026-07-17/18) — **#65a DONE** (`daed897`, wired real chat persistence onto the previously-dormant
> `CoachConversation/Message/ToolCall` Room tables) and **#65b DONE** (token/cost usage tracking +
> `MIGRATION_9_10` + `CoachUsageSheet`), both runtime-verified on `emulator-5554` including a real
> v9→v10 schema upgrade over existing data. #65b also fixed a real (non-iOS-parity) bug found along
> the way: `OpenAIResponse.parse` threw on any real API response with output content — see the #65b
> row for detail. **Next: #65c tool-call trace persistence + UI** (small — #65a/b already write bare
> `toolName`/`outputJSON` tool-call rows; just needs `label`/`statusRaw`/`sequence` columns + a
> trace-disclosure Composable), then #65d (environment/weather context) and #65f (variety hints +
> notification sleep-gating). #65e (context-budget compact mode) has no Android equivalent yet and
> should stay SKIPped unless a constrained/offline provider is added.
>
