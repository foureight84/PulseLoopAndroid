...
Status: **Phases 0–5 complete** (2026-08-16) — vitals, sleep, daily activity, workouts
+ GPS route, and the beyond-iOS set (blood pressure, blood glucose, respiratory rate,
VO₂max, resting HR, nutrition) all export; 16 `WRITE_*` permissions (0 `READ_*`), and the
VITALS group's BP/glucose/resp/VO₂ kinds plus the RESTING_HR and NUTRITION groups are live.
Remaining: Phase 6 (lifecycle, removal, docs — "remove PulseLoop data", grant/revocation
watermark resets + the four new kinds → VITALS mapping, archive restore, ios-sync.md
